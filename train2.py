...
# Save dataset heatmap
plt.figure(figsize=(12,10))
sns.heatmap(df.corr(), cmap='coolwarm', annot=False)
plt.title("Dataset Feature Correlation Heatmap")
output_dir = "model_output"
os.makedirs(output_dir, exist_ok=True)
plt.savefig(os.path.join(output_dir, "dataset_heatmap.jpeg"), format='jpeg')
plt.close()
...
joblib.dump(X_train_sel, os.path.join(output_dir, "X_train_selected.pkl"))

# SHAP Explanation (Optional)
try:
    # Use a sample of your training data as masker (background dataset)
    X_background = pd.DataFrame(X_train_sel).sample(100, random_state=42)
    ...
    plt.savefig(os.path.join(output_dir, "shap_summary_plot.jpeg"), format='jpeg')
    plt.close()
